...
export default function Slots3x3() {
  ...
  const [pos, setPos] = useState<number[]>(
    () => [Math.floor(Math.random() * REEL_LENGTH), Math.floor(Math.random() * REEL_LENGTH), Math.floor(Math.random() * REEL_LENGTH)]
  );
  ...
  const [transitionDurations, setTransitionDurations] = useState<number[]>([0, 0, 0]);
  ...
  const startSpin = () => {
    ...
    for (let col = 0; col < 3; col++) {
      ...
      // schedule stop
      const stopTime = baseStopDelay + col * gapBetweenStops + Math.floor(Math.random() * 300);
      ...
      timeoutsRef.current[col] = window.setTimeout(() => {
        ...
        const finalTopIndex = (finalMiddleIndex - 1 + REEL_LENGTH) % REEL_LENGTH; // top = middle -1 (porque temos 3 visíveis)
        ...
        if (col === 2) {
          ...
          timeoutsRef.current[3] = window.setTimeout(() => {
            ...
            // contar matches
            if (finalMiddle[0] === finalMiddle[1] && finalMiddle[1] === finalMiddle[2]) {
  ...
  return (
    <div className="bg-slate-900 min-h-screen flex flex-col items-center justify-center text-white p-6">
              ...
              >
                ...
                >
                  ...
                  {reels[col].map((s, i) => (
                    ...
                    >
                      <div className={`w-full h-full flex items-center justify-center text-4xl ${i === ( (pos[col]+1)%REEL_LENGTH) ? "scale-110 font-semibold" : ""}`}>
                        {s}
                      </div>
                    </div>
                  ))}
                  ...
                  {reels[col].slice(0, VISIBLE).map((s, i) => (
                    <div key={`${col}-wrap-${i}`} className="flex items-center justify-center" style={{ height: `${CELL_HEIGHT}px` }}>
                      <div className="w-full h-full flex items-center justify-center text-4xl">{s}</div>
                    </div>
                  ))}
                </div>
              </div>
      ...
      <p className="mt-4 text-sm text-slate-400 max-w-xl text-center">
        Cada coluna usa um "reel strip" com <strong>{REEL_LENGTH}</strong> stops. Isso imita roletas reais — em vez de sortear cada célula isoladamente, a coluna gira e para mostrando 3 símbolos visíveis (top/middle/bottom). Você pode ajustar <code>REEL_LENGTH</code> para simular máquinas clássicas (~20–30 stops) ou vídeo-slots modernos (32–78+).
      </p>
    </div>
  );
}